    ...
    $.fn.rotatingSlider = function(options){
        var rotatingSlider = {
            init: function(el){
                ...
                this.slideAngle = 60 / this.$slides.length;
            ...
            handleDragEnd: function(e){
                ...
                if(this.currentlyDragging){
                    ...
                    /*영상 재생 순서*/
                    var video = $(".slides .short-video");
                    var videoLong = $(".slides .long-video");
                    var arrVideo = [30,20,10,0,-10,-20];

                    for(var i=0;i<arrVideo.length;i++){
                        if(this.currentRotationAngle == arrVideo[i]){
                            $(".slides li").removeClass("on");
                            $(".slides li").eq(i).addClass("on");

                            video.trigger("pause");
                            video.eq(i).trigger("play");
                            
                            videoLong.removeClass("active").fadeOut();
                            ...
                            $(".slides .play-btn").fadeOut();
                            $(".slides .play-btn").eq(i).fadeIn();
                            $(".slides .btn").removeClass("click").fadeIn();
                        }
                    }
                }
            },
            ...
            renderSlider: function(){
                ...
                var $slide = $(el);
                    ...
                    $slide.css('transform', 'translateX(-50%) rotate('+this.slideAngle * (i-3) +'deg) translateY(-'+ upperArcHeight +'px)');
            ...
            rotate: function(){
                ...
                this.rotateTimeoutId = setTimeout(function(){
                    ...
                    //     this.$slidesContainer.css('transform', 'translateX(-50%) rotate('+this.currentRotationAngle+'deg)');
                    // }
                    if(this.currentRotationAngle >= 30){
                        this.currentRotationAngle = 30;
                        this.$slidesContainer.css('transform', 'translateX(-50%) rotate('+this.currentRotationAngle+'deg)');
                        // this.$slidesContainer.css('transition', 'all 0.8s');
                    }else if(this.currentRotationAngle <= -20){
                        this.currentRotationAngle = -20;
                        this.$slidesContainer.css('transform', 'translateX(-50%) rotate('+this.currentRotationAngle+'deg)');
                        // this.$slidesContainer.css('transition', 'all 0.8s');
                    }
