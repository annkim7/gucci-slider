        ...
        var circleSlider = {
            ...
            handleDragEnd: function(e){
                ...
                if(this.currentlyDragging){
                    ...
                    /*탭과 연동*/
                    var tabCircle = $(".palette_btn");                    
                    
                    for(var i=0;i<tabCircle.length;i++){
                        var tabTran = 40-(i*20);
                        // var tabTran = 280-(i*140);
                        var tabI = -72*i;
                        var beforeI = -63 + (-72*i);

                        if(this.currentRotationAngle == tabI){
                            $(".palette .tab").css({
                                transform: 'translateX(' + tabTran + '%)'
                            });
                        }

                        if(this.currentRotationAngle == beforeI){
                            $(".palette .tab").css({
                                transform: 'translateX(' + tabTran + '%)'
                            });
                        }

                        if(beforeI < this.currentRotationAngle && this.currentRotationAngle <= tabI){
                            $(".palette .tab").css({
                                transform: 'translateX(' + tabTran + '%)'
                            });
                        }
                    ...
                    for(var i=0;i<chipCircle.length;i++){
                        var chipI = -9*i;
                        var beforeI = -9*(i-1);

                        if(this.currentRotationAngle == chipI){
                            $(".chip").css({transform: 'scale(1)'});
                            $(".chip").eq(i).css({transform: 'scale(1.3)'});
                            $(".chip").not($(".chip").eq(i)).css({transform: 'scale(1)'});

                            $(".chip-txt").removeClass("on");
                            $(".chip-txt").eq(i).addClass("on");
                            $(".chip-txt").not($(".chip-txt").eq(i)).removeClass("on");
                        }

                        if(beforeI < this.currentRotationAngle && this.currentRotationAngle <= chipI){
                            $(".chip").css({transform: 'scale(1)'});
                            $(".chip").eq(i).css({transform: 'scale(1.3)'});
                            $(".chip").not($(".chip").eq(i)).css({transform: 'scale(1)'});
            ...
            rotate: function(){
                ...
                this.$slidesContainer.css('transition', 'transform '+(this.settings.rotationSpeed/800)+'s ease-in-out');
                this.$slidesContainer.css('transform', 'translateX(-50%) rotate('+this.currentRotationAngle+'deg)');
                
                this.rotateTimeoutId = setTimeout(function(){
                    this.currentlyRotating = false;
                    this.$slidesContainer.css('transition', 'all 0.8s');

                    /* keep currentRotationAngle between -360 and 360 */
                    if(this.currentRotationAngle >= 0){
                        this.currentRotationAngle = 0;
                        this.$slidesContainer.css('transform', 'translateX(-50%) rotate(0deg)');
                    }else if(this.currentRotationAngle <= -351){
                        this.currentRotationAngle = -351;
                        this.$slidesContainer.css('transform', 'translateX(-50%) rotate(-351deg)');
                    }
            ...
            validateMarkup: function(){
                if(
                    this.$slider.hasClass('circle-slider') &&
